...
export default async function handler(req, res) {
  ...
  try {
    if (req.method === 'POST') {
      ...
      const response = await fetch(`${SUPABASE_URL}/rest/v1/public.client_requests`, {
        method: 'POST',
        headers: {
    ...
    if (req.method === 'GET') {
      ...
      const response = await fetch(`${SUPABASE_URL}/rest/v1/public.client_requests?${query}`, {
        method: 'GET',
        headers: {
          'apikey': SUPABASE_KEY,
          'Authorization': `Bearer ${SUPABASE_KEY}`,
    ...
    if (req.method === 'PATCH') {
      ...
      const response = await fetch(`${SUPABASE_URL}/rest/v1/public.client_requests?id=eq.${id}`, {
        method: 'PATCH',
        headers: {
          'Content-Type': 'application/json',
          'apikey': SUPABASE_KEY,
